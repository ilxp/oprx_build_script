...
m = Map("udpxy", "udpxy", translate("udpxy是UDP到HTTP的多播流量中继守护程序，您可以在此处配置设置。"))
...
max_clients=s:option(Value, "max_clients", translate("最大客户数"))
max_clients.rmempty = true
max_clients.datatype = "range(1, 5000)"

log_file=s:option(Value, "log_file", translate("日志文件"))
...
buffer_messages=s:option(Value, "buffer_messages", translate("缓冲信息"))
...
nice_increment=s:option(Value, "nice_increment", translate("增量"))
nice_increment.rmempty = true
nice_increment.datatype = "or(and(max(-1), integer),and(min(1), integer))"

mcsub_renew=s:option(Value, "mcsub_renew", translate("组播订阅续订"))
